...
impl Hamming74 {
    ...
    pub fn decode_nibble(codeword: [bool; 7]) -> MuraResult<u8> {
        ...
        if syndrome != 0 {
            ...
            if error_pos < 1 || error_pos > 7 {
                ...
                // btw .into() converts object to a different type
    ...
    pub fn decode(encoded: &[u8], original_len: usize) -> MuraResult<Vec<u8>> {
        ...
        for i in 0..original_len{
            ...
            // i just manually got the bit positions, there is probably a better way to do this
...
pub fn save_model(
    ...
    // also extend_from_slice: https://doc.rust-lang.org/std/vec/struct.Vec.html
    // for extend_from_slice, this easier to append to a vec and doesn't change the original
...
pub fn load_model(
    ...
    // verify CRC
    // let's say we have the decimal version of 0XDEADBEEF, which is 3735928559.
    // but then we actual get something that is similar but not really with the computation
    // just use this to see the actual hexcode instead of the decimal form
